...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class NoiseAndGrain : PostEffectsBase
	{
        ...
        private static float Node_AMOUNT = 64.0f;
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination)
		{
            ...
            if (dx11Grain && supportDX11)
			{
                ...
                if (softness > Mathf.Epsilon)
                {
                    ...
                    DrawNoiseQuadTile (source, rt, dx11NoiseMaterial, noiseTexture, monochrome ? 3 : 2);
                    dx11NoiseMaterial.SetTexture("_NoiseTex", rt);
                    Graphics.Blit(source, destination, dx11NoiseMaterial, 4);
                    RenderTexture.ReleaseTemporary(rt);
                }
                else
                    DrawNoiseQuadTile (source, destination, dx11NoiseMaterial, noiseTexture, (monochrome ? 1 : 0));
            }
            else
			{
                ...
                if (softness > Mathf.Epsilon)
                {
                    ...
                    DrawNoiseQuadTile (source, rt2, noiseMaterial, noiseTexture, 2);
                    noiseMaterial.SetTexture("_NoiseTex", rt2);
                    Graphics.Blit(source, destination, noiseMaterial, 1);
                    RenderTexture.ReleaseTemporary(rt2);
                }
                else
                    DrawNoiseQuadTile (source, destination, noiseMaterial, noiseTexture, 0);
            }
        }

        static void DrawNoiseQuadTile (RenderTexture source, RenderTexture dest, Material fxMaterial, Texture2D noise, int passNr)
		{
            RenderTexture.active = dest;

            float noiseSize = (noise.width * 1.0f);
            float subDs = (1.0f * source.width) / Node_AMOUNT;
            ...
            float texNode = noiseSize / (noise.width * 1.0f);
            ...
            for (float x1 = 0.0f; x1 < 1.0f; x1 += stepSizeX)
			{
                for (float y1 = 0.0f; y1 < 1.0f; y1 += stepSizeY)
				{
                    ...
                    float texNodeMod = 1.0f / noiseSize;

                    GL.MultiTexCoord2 (0, tcXStart, tcYStart);
                    GL.MultiTexCoord2 (1, 0.0f, 0.0f);
                    //GL.Color( c );
                    GL.Vertex3 (x1, y1, 0.1f);
                    GL.MultiTexCoord2 (0, tcXStart + texNode * texNodeMod, tcYStart);
                    GL.MultiTexCoord2 (1, 1.0f, 0.0f);
                    //GL.Color( c );
                    GL.Vertex3 (x1 + stepSizeX, y1, 0.1f);
                    GL.MultiTexCoord2 (0, tcXStart + texNode * texNodeMod, tcYStart + texNode * texNodeMod);
                    GL.MultiTexCoord2 (1, 1.0f, 1.0f);
                    //GL.Color( c );
                    GL.Vertex3 (x1 + stepSizeX, y1 + stepSizeY, 0.1f);
                    GL.MultiTexCoord2 (0, tcXStart, tcYStart + texNode * texNodeMod);
