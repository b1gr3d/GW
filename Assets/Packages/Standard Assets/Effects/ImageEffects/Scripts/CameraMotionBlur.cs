    ...
    public class CameraMotionBlur : PostEffectsBase
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination) {
            ...
            if (filterType == MotionBlurFilter.Reconstruction || fallbackFromDX11 || filterType == MotionBlurFilter.ReconstructionDisc) {
                ...
                NodeWidth = divRoundUp (velBuffer.width, (int) maxVelocity);
                NodeHeight = divRoundUp (velBuffer.height, (int) maxVelocity);
                _maxVelocity = velBuffer.width/NodeWidth;
            }
            else {
                NodeWidth = divRoundUp (velBuffer.width, (int) maxVelocity);
                NodeHeight = divRoundUp (velBuffer.height, (int) maxVelocity);
            ...
            RenderTexture NodeMax  = RenderTexture.GetTemporary (NodeWidth, NodeHeight, 0, rtFormat);
            RenderTexture neighbourMax  = RenderTexture.GetTemporary (NodeWidth, NodeHeight, 0, rtFormat);
            velBuffer.filterMode = FilterMode.Point;
            NodeMax.filterMode = FilterMode.Point;
            ...
            NodeMax.wrapMode = TextureWrapMode.Clamp;
            ...
            motionBlurMaterial.SetTexture ("_NodeTexDebug", NodeMax);
            ...
            // debug vel buffer:
            if (showVelocity) {
                // generate Node max and neighbour max
                //Graphics.Blit (velBuffer, NodeMax, motionBlurMaterial, 2);
                //Graphics.Blit (NodeMax, neighbourMax, motionBlurMaterial, 3);
                ...
                if (filterType == MotionBlurFilter.ReconstructionDX11 && !fallbackFromDX11) {
                    ...
                    // generate Node max and neighbour max
                    Graphics.Blit (velBuffer, NodeMax, dx11MotionBlurMaterial, 0);
                    Graphics.Blit (NodeMax, neighbourMax, dx11MotionBlurMaterial, 1);
            ...
            RenderTexture.ReleaseTemporary (NodeMax);
