 ...
 */
 ...
 Shader "Hidden/CameraMotionBlurDX11" {
	...
	// find dominant velocity in each Node
	float4 NodeMax(v2f i) : SV_Target
	{
		float2 Nodemax = float2(0.0, 0.0);
		...
		for(int y=0; y<(int)_MaxRadiusOrKInPaper; y++) {
			for(int x=0; x<(int)_MaxRadiusOrKInPaper; x++) {
				...
				Nodemax = vmax(Nodemax, v);
		  	}
  	  	}
  	  	return float4(Nodemax, 0, 1);
	}

	// find maximum velocity in any adjacent Node
