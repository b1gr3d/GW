...
{
    public WeaponType type;                    // Weapon type
    ...
    {   
        ship.GetComponentInChildren<WeaponPool>().Despawn(transform);
    }
            ...
            if (!isFXSpawned)
            {
                ...
                switch (type)
                {
                    case WeaponType.Vulcan:
                        ship.GetComponentInChildren<WeaponController>().VulcanImpact(hitPoint.point + hitPoint.normal * 0.2f);
                        //ApplyForce(2.5f);
                        break;
